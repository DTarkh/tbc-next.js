...
useEffect(() => {
  ...
  return (
    <div className="relative w-16 h-8 flex items-center dark:bg-[#FCA311] bg-[#14213D] cursor-pointer rounded-full p-1" onClick={() =>setDarkMode(!darkMode)}>
        <FaMoon className="text-white" size={18}/>
        <div className="absolute bg-white w-6 h-6 rounded-full shadow-md " style={darkMode ? {left:"2px"} : {right: "2px"}}></div>
        <BsSunFill className="text-yellow-100 ml-auto" size={18} />
    </div>
  )
}
